...
exports.checkSessionAuth = function(req, container, callback) {
    ...
    if(!req.data.session.appid)
        return callback(new Error('Application ID is required'));

    container.getService('MONGODB').then(function(service) {

        service.send('findOne', {collectionName : req.data.session.appid, query : {where : {_className : '_Users', sessionToken : req.headers['x-noserv-session-token']}}}, function(err, res) {
...
exports.checkMasterKey = function(req, container, callback) {
    ...
    if(!req.headers['x-noserv-master-key'] || req.headers['x-noserv-master-key'] != req.data.session.masterKey)
